...
export function createDevServer(config: HttpServerConfig) {
  ...
  server.addRoute('GET', '/(index.html)?', serveIndex);
 ...
 */
function serveIndex(req: Request, res: Response): Promise<void>  {
  const htmlFile = req.filePath.indexOf('index.html') === -1 ?
    path.join(req.filePath, 'index.html') :
    req.filePath;

  return readFilePromise(htmlFile).then((content: Buffer) => {
 ...
 */
function serveAppScriptAsset(req: Request, res: Response): Promise<void> {
  const fileName = req.params[0];
  const filePath = path.join(__dirname, '..', 'bin', fileName);

  return readFilePromise(filePath).then((content: Buffer) => {
    res.writeHead(200, {
 ...
 */
function serveCordovaJS(req: Request, res: Response): Promise<void>  {
  res.writeHead(200, {
